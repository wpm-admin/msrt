...
class ModelLocalisationConditionStatus extends Model {
	public function addConditionStatus($data) {
		foreach ($data['condition_status'] as $language_id => $value) {
			if (isset($condition_status_id)) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "condition_status SET condition_status_id = '" . (int)$condition_status_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "'");
			} else {
				$this->db->query("INSERT INTO " . DB_PREFIX . "condition_status SET language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "'");

				$condition_status_id = $this->db->getLastId();
			}
		}

		$this->cache->delete('condition_status');
		
		return $condition_status_id;
	}

	public function editConditionStatus($condition_status_id, $data) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "condition_status WHERE condition_status_id = '" . (int)$condition_status_id . "'");
	...
	public function deleteConditionStatus($condition_status_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "condition_status WHERE condition_status_id = '" . (int)$condition_status_id . "'");
