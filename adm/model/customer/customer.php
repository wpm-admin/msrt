...
class ModelCustomerCustomer extends Model {
	public function addCustomer($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode(array())) . "', newsletter = '" . (int)$data['newsletter'] . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password2'])))) . "', status = '" . (int)$data['status'] . "', safe = '" . (int)$data['safe'] . "', date_added = NOW()");

		$customer_id = $this->db->getLastId();

		if ($data['faccount_number']) {
			$this->db->query("UPDATE " . DB_PREFIX . "customer SET account_number = '" . $this->db->escape($data['account_number']) . "' WHERE customer_id = '" . (int)$customer_id . "'");
		}

		if ($data['telephone22']) {
			$this->db->query("UPDATE " . DB_PREFIX . "customer SET telephone2 = '" . $this->db->escape($data['telephone22']) . "' WHERE customer_id = '" . (int)$customer_id . "'");
		}
	...
	public function editCustomer($customer_id, $data) {
		...
		if ($data['password2'] AND $data['password2'] == $data['confirm']) {
			$this->db->query("UPDATE " . DB_PREFIX . "customer SET salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password2'])))) . "' WHERE customer_id = '" . (int)$customer_id . "'");
		}
	...
	public function getCustomerUTM($customer_id) {
		$query = $this->db->query("SELECT order_id,
								  ...
								  WHERE customer_id = '" . (int)$customer_id . "' AND utm_source IS NOT NULL
