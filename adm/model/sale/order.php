...
class ModelSaleOrder extends Model {
	...
	public function getOrders($data = array()) {
		...
		if (!empty($data['filter_order_status'])) {
			...
			$sql .= " WHERE 1 "; //o.order_status_id > '0'";
		}
		...
		if (!empty($data['filter_utm'])) {
			$model = utf8_strtolower($data['filter_utm']);
			
			$sql .= "AND (LOWER(o.utm_source) LIKE '%" . $this->db->escape(implode('%', $model)) . "%'";
			$sql .= "OR LOWER(o.utm_medium) LIKE '%" . $this->db->escape(implode('%', $model)) . "%'";
			$sql .= "OR LOWER(o.utm_campaign) LIKE '%" . $this->db->escape(implode('%', $model)) . "%'";
			$sql .= "OR LOWER(o.utm_content) LIKE '%" . $this->db->escape(implode('%', $model)) . "%'";
			$sql .= "OR LOWER(o.utm_term) LIKE '%" . $this->db->escape(implode('%', $model)) . "%'";
			$sql .= "OR LOWER(o.ad_name) LIKE '%" . $this->db->escape(implode('%', $model)) . "%')";
		}
...
//die($sql);
	...
	public function getTotalSales($data = array()) {
		$sql = "SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` o";
