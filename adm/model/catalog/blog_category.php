...
class ModelCatalogBlogCategory extends Model {
	public function addCategory($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "blog_category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW(), date_added = NOW()");
		...
		if (isset($data['blog_category_filter'])) {
			foreach ($data['blog_category_filter'] as $filter_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "blog_category_filter SET blog_category_id = '" . (int)$blog_category_id . "', filter_id = '" . (int)$filter_id . "'");
			}
		}
	...
	public function editCategory($blog_category_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "blog_category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE blog_category_id = '" . (int)$blog_category_id . "'");
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "blog_category_filter WHERE blog_category_id = '" . (int)$blog_category_id . "'");
	...
	public function deleteCategory($blog_category_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "blog_category_to_store WHERE blog_category_id = '" . (int)$blog_category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "blog_category_to_layout WHERE blog_category_id = '" . (int)$blog_category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "blog_product_to_category WHERE blog_category_id = '" . (int)$blog_category_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "seo_url WHERE query = 'blog_category_id=" . (int)$blog_category_id . "'");
	...
	public function repairCategories($parent_id = 0) {
		...
		foreach ($query->rows as $blog_category) {
			...
			$this->repairCategories($blog_category['blog_category_id']);
		}
	}
	...
	public function getCategories($data = array()) {
		...
		$return = array();
		foreach( $query->rows as $blog_category){
			$return[$blog_category['blog_category_id']] = $blog_category;
		}
