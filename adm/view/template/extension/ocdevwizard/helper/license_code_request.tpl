<div id="service-modal-body" class="mw500 service-modal-body">
				...
				<div class="panel-body" style="padding-top: 0;">
					<form method="post" enctype="multipart/form-data" class="form-horizontal">
						<div class="row">
							...
							<div class="form-group required">
								<label class="col-sm-12 control-label"><?php echo $entry_domain; ?><i class="fa fa-question-circle show-explanation" data-toggle="tooltip" title="<?php echo $text_open_explanation; ?>"onclick="show_explanation({a:this})"></i></label>
								<div class="col-sm-12">
									<input type="text" name="domain" value="" class="form-control" id="modal-domain" autocomplete="off"/>
									<div class="alert alert-info" style="display: none;"><i class="fa fa-exclamation-circle" aria-hidden="true"></i> <?php echo $entry_domain_faq; ?></div>
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-12 control-label" style="text-align:left !important;padding-top:0">
									<input type="checkbox" name="test_domain_status" onchange="$(this).parent().parent().next().toggle();"/> <?php echo $entry_test_domain_faq_2; ?>
								</label>
							</div>
							<div class="form-group" style="display: none;">
								<label class="col-sm-12 control-label"><?php echo $entry_test_domain; ?><i class="fa fa-question-circle show-explanation" data-toggle="tooltip" title="<?php echo $text_open_explanation; ?>"onclick="show_explanation({a:this})"></i></label>
								<div class="col-sm-12">
									<input type="text" name="test_domain" value="" class="form-control" id="modal-test-domain" autocomplete="off"/>
									<div class="alert alert-info" style="display: none;"><i class="fa fa-exclamation-circle" aria-hidden="true"></i> <?php echo $entry_test_domain_faq_1; ?></div>
								</div>
							</div>
						</div>
					</form>
	...
	<div class="modal-footer" id="service-modal-footer">
		...
		<button class="btn btn-success button-loading" onclick="submit_license_code_request(this, 'close');"><span class="hidden-md hidden-lg"><i class="fa fa-save"></i> </span><span class="hidden-xs hidden-sm"><?php echo $button_send; ?></span></button>
	</div>
</div>
<script>
	$('#service-modal-data input[name=\'module_name\']').val($('#content h1').html());

	function submit_license_code_request(element, after_action = '') {
    $.ajax({
      type: 'post',
      url: 'index.php?route=extension/ocdevwizard/<?php echo $_name; ?>/license_code_request_action&<?php echo $token; ?>',
