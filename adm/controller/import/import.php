...
class ControllerImportImport extends Controller {
	...
	public function index() {
		...
		*/
		...
		$data['href_import_special'] = $this->url->link('import/import/import_special', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$data['href_import_customer_group'] = $this->url->link('import/import/import_customer_group', 'user_token=' . $this->session->data['user_token'] . $url, true);
	...
	public function export_excel_photos(){
		...
		*/
		...
		$products = $this->model_import_import->getProductsShort($this->request->post);
		...
		$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, 'product_id');
		$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, 'CodeManual');
		$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, 'Product Number');
		$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, 'English');
		$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, 'Price');
		$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, 'Availability');
		$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, 'Status');
		$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, 'image');
		...
		foreach($products as $product){
			...
			$product_images = $products = $this->model_catalog_product->getProductImages($product['product_id']);
			if($product['image']){
			...
			foreach($images as $index => $image){
				$images[$index] = $this->model_tool_image->resize($image, 1000, 800);
			}
		
			
			$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, $product['product_id']);
			$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, $product['model']);
			$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, $product['sku']);
			$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, $this->decodeHtml($product['name']));
			$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, $product['price']);
			$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, $product['quantity']);
			$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, $statuses[(int)$product['status']]);
			$objPHPExcel->getActiveSheet()->setCellValue($ABC[$r++].$i, implode(';', $images));
		...
		header('Content-Type: application/vnd.ms-excel'); 
		header('Content-Disposition: attachment;filename="maseratinet_images.xls"'); 
		header('Cache-Control: max-age=0'); 
	...
	public function export_excel(){
		...
		if(isset($this->request->post['picture_only']) AND (int)$this->request->post['picture_only'] == 1){
			$this->export_excel_photos();
			return false;
		}
		...
		if(count($export) > 0){
			...
			if(isset($undiagramme['list']) AND count($undiagramme['list']) > 0){
				foreach($undiagramme['list'] as $row){
					...
					if(isset($post['column'][20])){
						$objPHPExcel->getActiveSheet()->setCellValue($ABC[$bukva++].$i, (isset($row['manufacturer_id']) AND isset($manufacturers[(int)$row['manufacturer_id']])) ? $manufacturers[(int)$row['manufacturer_id']]['name'] : '');
					}
	...
	public function import_special(){
		...
		//Расшифровываем строчку со скидкой для группы клиентов и проставляем этой группе скидку. Если процент то процент
		...
		if(isset($_FILES['file']['tmp_name']) AND $_FILES['file']['tmp_name'] != ''){
			...
			while($i < $worksheetRows){
				...
				foreach(array('A', 'B') AS $bukva){
					$value = trim($worksheet->getCell($bukva.$i )->getValue());
					
					if($value != ''){
						
						$group = 'none';
						
						if(strpos($value, 'RESALE') !== false){
							$tmp = explode('RESALE', $value);
							$group = 2; //customer_group_id for Resale
						}elseif(strpos($value, 'MEMBER') !== false){
							$tmp = explode('MEMBER', $value);
							$group = 3;
						}
						...
						if(strpos($price, '$') !== false){
							...
							$price = explode(' ', $price);
							$price = trim(array_pop($price));
						}else{
							$price = 0;
						}
						
						
						$products[trim($tmp[0],'-')][$group] = $price;
					}
				}
			...
			foreach($products as $model => $values){
				
				$query = $this->db->query("SELECT product_id, price FROM " . DB_PREFIX . "product WHERE model = '".$this->db->escape($model)."' LIMIT 1");
				...
				if($query->num_rows){
					...
					foreach($values as $customer_group_id => $special){
						...
						if((float)$price > 0){
							...
							if(strpos($special, '%') !== false){
								if((float)$special > 0){
									$this->db->query("INSERT INTO " . DB_PREFIX . "product_special SET price_prefix='%', product_id = '".(int)$product_id."', customer_group_id = '".(int)$customer_group_id."', price = '".(float)$special."'");
								}
							}elseif((float)$special > 0){
								$this->db->query("INSERT INTO " . DB_PREFIX . "product_special SET price_prefix='=', product_id = '".(int)$product_id."', customer_group_id = '".(int)$customer_group_id."', price = '".(float)$special."'");
							}							
	...
	public function import_customer_group(){
			
		$this->load->model('import/import');
			
		if(isset($_FILES['file']['tmp_name']) AND $_FILES['file']['tmp_name'] != '' AND isset($this->request->post['customer_group_id']) AND (int)$this->request->post['customer_group_id'] > 0){
			...
			while($i < $worksheetRows){
				...
				if(trim($phone) != '' AND strlen($phone) > 7){
					
					$sql = "UPDATE  " . DB_PREFIX . "customer SET `customer_group_id`='".(int)$this->request->post['customer_group_id']."' WHERE telephone LIKE '%".$this->db->escape(utf8_strtolower($phone))."%'";
					//echo '<br>'.$sql;
					$this->db->query($sql);
				}
	...
	*/
		...
		if(isset($_FILES['file']['tmp_name']) AND $_FILES['file']['tmp_name'] != ''){
				...
				foreach($rows as $index => $row){
					...
					if(isset($row['Marque']) AND isset($row['Model']) AND isset($row['SubCategory (Alias)']) AND isset($row['Diagram'])){
						$diagram_name = $row['Marque'] . ' ' . (string)$row['Model'] . ', ' . utf8_strtolower($row['SubCategory (Alias)']) . ' - ' . $row['Diagram'];

						//Убрано по прозьбе Николая 05,08,2024
						//Я так заплутаюсь. Можеш зробити нормально щоб враховували модель з поля модель а не з назви діаграми?
					...
					if(isset($row['image']) AND trim($row['image']) != '' AND trim($row['image']) != 'none' ){
						$this->model_import_import->updateOCProductImage($product_id, trim($row['image']));
					}
					else if(trim($row['image']) == 'none' ){
						$this->model_import_import->updateOCProductImage($product_id, 'none');
					}
	...
	private function _getVendors(){
		...
		$results = $this->model_vendor_vendor->getVendors();
		...
		*/
		foreach ($results as $result) { 
			$return[$result['vendor_id']] = array('vendor_id' => $result['vendor_id'], 'name' => $result['lastname']);
		}
