...
class ControllerExtensionModuleLiveSearch extends Controller {
	...
	public function index() {
		...
		$data['action'] = $this->url->link($extension . 'module/live_search', 'user_token=' . $this->session->data['user_token'], 'SSL');
		if ($extension) {
		...
		$data['entry_show_add_button'] = $this->language->get('entry_show_add_button');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link($extension . 'module/live_search', 'user_token=' . $this->session->data['user_token'], 'SSL')
		);
		...
		// 顯示加入購物車按鈕
		if (isset($this->request->post['live_search_show_add_button'])) {
			$data['live_search_show_add_button'] = $this->request->post['live_search_show_add_button'];
		} else {
			$data['live_search_show_add_button'] = $this->config->get('live_search_show_add_button');
		}
	...
	public function install() {
		...
		$this->model_setting_event->addEvent('ajax_live_search', 'catalog/view/*/template/common/header/after', 'extension/module/live_search/injectLiveSearch');
		// $this->model_extension_event->addEvent('ajax_live_search', 'catalog/view/common/header/after', 'extension/module/live_search/injectLiveSearch');
	}

	public function uninstall() {
		$this->load->model('setting/event');
		$this->model_setting_event->deleteEvent('ajax_live_search');
	}
}
?>