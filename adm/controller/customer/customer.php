...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['customers'][] = array(
				...
				'name'           => $result['name'],
				'utm'          => $this->model_customer_customer->getCustomerUTM($result['customer_id']),
				'email'          => $result['email'],
		...
		$data['sort_account_number'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . '&sort=c.account_number' . $url, true);
		$data['sort_shipping'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . '&sort=c.shipping' . $url, true);
	...
	protected function getForm() {
		...
		if (isset($this->error['password2'])) {
			$data['error_password'] = $this->error['password2'];
		} else {
			$data['error_password'] = '';
		}
		...
		if (isset($this->request->post['shipping'])) {
			...
			$data['shipping'] = $customer_info['shipping'];
		} else {
			$data['shipping'] = '';
		}
		...
		if (isset($this->request->post['telephone22'])) {
			$data['telephone22'] = $this->request->post['telephone22'];
		} elseif (!empty($customer_info)) {
			$data['telephone22'] = $customer_info['telephone2'];
		} else {
			$data['telephone22'] = '';
		}
		...
		$data['utms'] = $this->model_customer_customer->getCustomerUTM($this->request->get['customer_id']);
		...
		if (isset($this->request->post['password2'])) {
			$data['password2'] = $this->request->post['password2'];
		} else {
			$data['password2'] = '';
		}
	...
	protected function validateForm() {
		...
		/*
		if ($this->request->post['password2'] || (!isset($this->request->get['customer_id']))) {
			if ((utf8_strlen(html_entity_decode($this->request->post['password2'], ENT_QUOTES, 'UTF-8')) < 4) || (utf8_strlen(html_entity_decode($this->request->post['password2'], ENT_QUOTES, 'UTF-8')) > 40)) {
				$this->error['password2'] = $this->language->get('error_password');
			}

			if ($this->request->post['password'] != $this->request->post['confirm'] AND $this->request->post['confirm'] != '') {
