...
class ControllerSaleOrder extends Controller {
	...
	public function get_utm(){
		...
		foreach (range('P', $objPHPExcel->getActiveSheet()->getHighestDataColumn()) as $col) {
			$objPHPExcel->getActiveSheet() ->getColumnDimension($col) ->setAutoSize(true);
		} 
	
		$objPHPExcel->getActiveSheet()->setCellValue('A'.$i, 'Order');
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$i, 'Date');
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$i, 'Total');
		$objPHPExcel->getActiveSheet()->setCellValue('D'.$i, 'Customer');
		$objPHPExcel->getActiveSheet()->setCellValue('E'.$i, 'Phone');
		$objPHPExcel->getActiveSheet()->setCellValue('F'.$i, 'email');
		$objPHPExcel->getActiveSheet()->setCellValue('G'.$i, 'utm_source');
		$objPHPExcel->getActiveSheet()->setCellValue('H'.$i, 'utm_medium');
		$objPHPExcel->getActiveSheet()->setCellValue('I'.$i, 'utm_campaign');
		$objPHPExcel->getActiveSheet()->setCellValue('J'.$i, 'utm_content');
		$objPHPExcel->getActiveSheet()->setCellValue('K'.$i, 'utm_term');
		$objPHPExcel->getActiveSheet()->setCellValue('L'.$i, 'ad_name');
		$i++;
  

		$orders = $this->model_sale_order->getUtmOrders();	

		foreach ($orders as $order) {
		
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$i, $order['order_id']);
			$objPHPExcel->getActiveSheet()->setCellValue('B'.$i, $order['date_added']);
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$i, $order['total']);
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$i, $order['firstname'] . ' ' . $order['lastname']);
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$i, $order['telephone']);
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$i, $order['email']);
			$objPHPExcel->getActiveSheet()->setCellValue('G'.$i, $order['utm_source']);
			$objPHPExcel->getActiveSheet()->setCellValue('H'.$i, $order['utm_medium']);
			$objPHPExcel->getActiveSheet()->setCellValue('I'.$i, $order['utm_campaign']);
			$objPHPExcel->getActiveSheet()->setCellValue('J'.$i, $order['utm_content']);
			$objPHPExcel->getActiveSheet()->setCellValue('K'.$i, $order['utm_term']);
			$objPHPExcel->getActiveSheet()->setCellValue('L'.$i, $order['ad_name']);
		...
		header('Content-Disposition: attachment;filename="utm_list-'.date('Y_m_d_H_i_s').'.xls"'); 
	...
	public function delete() {
		...
		if (isset($this->request->get['filter_utm'])) {
			$url .= '&filter_utm=' . $this->request->get['filter_utm'];
		}
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_utm'])) {
			$filter_utm = $this->request->get['filter_utm'];
		} else {
			$filter_utm = '';
		}
		...
		$data['invoice_pdf'] = $this->url->link('sale/order/invoice_pdf', 'user_token=' . $this->session->data['user_token'], true);
		...
		$data['get_utm'] = $this->url->link('sale/order/get_utm', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'shipping_date'      => (!empty($result['shipping_date']))? date('d/m/Y', strtotime($result['shipping_date'])) : '',
	...
	public function invoice_pdf(){
		...
		$html = $pdf = trim($this->getInvoice('order_invoice_pdf'));
		...
		$dompdf->render();
		$dompdf->stream('order-'.$order_id.'.pdf');
		exit();
		return true;
	...
	public function invoice() {
		...
		$this->response->setOutput($this->getInvoice());
	}
	public function getInvoice($tpl = 'order_invoice'){
		$this->load->language('sale/order');
		...
		return $this->load->view('sale/' . $tpl, $data);
	}
