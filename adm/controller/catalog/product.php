...
class ControllerCatalogProduct extends Controller {
	...
	public function update() {
		...
		if (isset($this->request->post['selected'])) {
			...
			if(isset($this->request->post['product_image']) AND trim($this->request->post['product_image']) != ''){
	...
	protected function getList() {
		...
		$data['filter_customer_id'] = $filter_customer_id;
			...
			$filter_sku = $filter_manufacturer = $filter_category = $filter_mark = $filter_vendor = false;
			if (isset($this->request->get['filter_sku'])) {
		...
		$data['redirect'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$data['update'] = $this->url->link('catalog/product/update', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['update_vendor'] = $this->url->link('catalog/product_edit/update_vendor', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$data['vendors'] = array();//$this->model_vendor_vendor->getVendors();
	
		$data['customers'] = $this->model_catalog_product->getCustomersInProduct();

		foreach($data['customers'] as $customer_id => $row){
			$data['customers'][$customer_id] = $this->model_customer_customer->getCustomer($customer_id);
		}
		...
		foreach ($results as $result) {
			...
			foreach ($product_specials  as $product_special) {
				if (($product_special['date_start'] == '0000-00-00' || strtotime($product_special['date_start']) < time()) && ($product_special['date_end'] == '0000-00-00' || strtotime($product_special['date_end']) > time())) {
					...
					}elseif($product_special['price_prefix'] == '+'){
						$product_special['price'] = $result['price'] - $product_special['price'];
					}elseif($product_special['price_prefix'] == '-'){
						$product_special['price'] = $result['price'] - $product_special['price'];
					}elseif($product_special['price_prefix'] == '%'){
						$product_special['price'] = $result['price'] / 100 * (100 - $product_special['price']);
					}else{
			...
			foreach($marks as $mark_id){
				$mark_names[] = $data['marks'][$mark_id]['name'];
			}
			...
			$data['products'][] = array(
				...
				'vendors' => array(),//$this->model_catalog_product->getProductVendors($result['product_id']),
				'cost'       => $result['cost'],
		...
		foreach($marks as $index => $row){
			if($row['parent_id'] == 0){
				$data['marks'][] = $data['my_marks'][$index] = $row;
				unset($marks[$index]);

				foreach($marks as $index1 => $row1){
					if($row1['parent_id'] == $index){
						$data['marks'][] = $data['my_marks'][$index]['parent'][$index1] = $row1;
						unset($marks[$index1]);

						foreach($marks as $index2 => $row2){
							if($row2['parent_id'] == $index1){
								$data['marks'][] = $data['my_marks'][$index]['parent'][$index1]['parent'][$index2] = $row2;
								unset($marks[$index2]);
		...
		$data['sort_sku'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.sku' . $url, true);
		$data['sort_manufacturer'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=md.name' . $url, true);
		$data['sort_category'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=cd.name' . $url, true);
		$data['sort_mark'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=mode.name' . $url, true);
		$data['sort_weight'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.weight' . $url, true);
		$data['sort_condition'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.condition' . $url, true);
		$data['sort_uom'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.uom' . $url, true);
		...
		$data['sort_cost'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.cost' . $url, true);
	...
	protected function getForm() {
		...
		*/
		...
		if (isset($this->request->post['date_off'])) {
			$data['date_off'] = $this->request->post['date_off'];
		} elseif (!empty($product_info)) {
			$data['date_off'] = $product_info['date_off'];
		} else {
			$data['date_off'] = '0000-00-00';
		}
		...
		foreach ($product_discounts as $product_discount) {
			$data['product_discounts'][] = array(
				...
				'price_prefix'             => $product_discount['price_prefix'],
		...
		foreach ($product_specials as $product_special) {
			$data['product_specials'][] = array(
				...
				'price_prefix'             => $product_special['price_prefix'],
		...
		$data['consignment_id'] = 0;
		
		if(isset($this->request->get['add_like_consignment'])){
			...
			$data['consignment_id'] = $this->request->get['consignment_id'];
			
			$data['cost'] = $consignment_info['price'];
			$data['price'] = $consignment_info['my_price'];
			
			$images = explode(';', $consignment_info['images']);
			...
			foreach($images as $image){
				...
				if($image != ''){
					...
					if(!is_dir($directory . '/' . $data['consignment_id'])){
						mkdir($directory . '/' . $data['consignment_id'], 0777);
						chmod($directory . '/' . $data['consignment_id'], 0777);
					}
	
					
					copy(DIR_UPLOAD.$f_name_origin, $directory . '/' . $data['consignment_id']. '/'.$f_name);
					
					if($count++ == 1){
						$data['def_image'] = $data['image'] = 'consignment/' . $data['consignment_id']. '/'.$f_name;
					}else{
						$product_images[] = array('image' => 'consignment/' . $data['consignment_id']. '/'.$f_name);
					}
				}
			}
			...
			foreach($data['languages'] as $language_id => $language){
				$data['product_description'][$language['language_id']]['name'] = $consignment_info['name'];
			}
			
			$category_info = $this->model_catalog_category->getCategory($consignment_info['category_id']);
			...
			$mark_info = $this->model_catalog_mark->getMark($consignment_info['mark_id']);
		...
		if (isset($this->request->post['image']) && is_file(DIR_IMAGE . $this->request->post['image'])) {
			...
			$data['thumb'] = $this->model_tool_image->resize($data['def_image'], 100, 100);
			$data['thumb_coordiname'] = $this->model_tool_image->resize($data['def_image'], 1000, 800);
		}
		...
		if (isset($this->request->post['image2']) && is_file(DIR_IMAGE . $this->request->post['image2'])) {
			...
			$data['thumb2'] = $this->model_tool_image->resize($data['def_image'], 100, 100);
		}

		$data['placeholder'] = $this->model_tool_image->resize($data['def_image'], 100, 100);
	...
	protected function validateForm() {
		...
		if (false and $this->request->post['product_seo_url']) {
	...
	public function get_diagram_list(){
