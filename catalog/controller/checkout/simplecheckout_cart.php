...
class ControllerCheckoutSimpleCheckoutCart extends SimpleController {
    ...
    public function index() {
        ...
        $this->_templateData['isLogged'] = $this->customer->isLogged();
        ...
        foreach ($products as $product) {
            ...
            if ($product['image']) {
                ...
                $image = $this->model_tool_image->resize('placeholder.png', $image_cart_width, $image_cart_height);
            }
            ...
            if ($version >= 200) {
                ...
                $this->_templateData['products'][] = array(
                    ...
                    'manuf'   => $product_info['manufacturer'],
                    'option'    => $option_data,
                    'recurring' => $recurring,
        ...
        $this->_templateData['btn_continue_href'] = $this->url->link('product/mark', 'mark_id=' . (int)$this->session->data["mark_id"]);
        
        
        if($this->model_tool_tool->isMobile()){
            $this->setOutputContent($this->renderPage('checkout/simplecheckout_cart_mob', $this->_templateData));    
        }else{
    ...
    private function validateCoupon() {
        ...
        //Тут відбувається перевірка купонів
