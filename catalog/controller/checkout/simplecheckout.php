...
class ControllerCheckoutSimpleCheckout extends SimpleController {
    ...
    public function index($args = null) {
        ...
        $data['btn_continue_href'] = $this->url->link('product/mark', 'mark_id=' . (int)$this->session->data["mark_id"]);
        
        //echo "<pre>";print_r(var_dump($this->session->data['shipping_address']));echo "</pre>";
        //die();
        
        //Определение группы участника по стране по ИП и переключение языка
        if (isset($this->session->data['shipping_address']['country_id']) AND isset($this->session->data['payment_address']['country_id'])) {
			$data['country_id'] = $this->session->data['shipping_address']['country_id'];
		} else {
			$ip = $_SERVER['REMOTE_ADDR'];
            if(@$ip_info = file_get_contents("http://www.geoplugin.net/json.gp?ip={$ip}")){
                ...
                if($country_info){
                    $this->session->data['shipping_address']['country_id'] = $data['country_id'] = $country_info['country_id'];
                    $this->session->data['payment_address']['country_id'] = $data['country_id'] = $country_info['country_id'];
                }else{
                    $this->session->data['shipping_address']['country_id'] = $data['country_id'] = $this->config->get('config_country_id');
                }
