...
class ModelCheckoutOrder extends Model {
	...
	public function addUtm($order_id){
		
		if (isset($this->request->cookie['utm_source'])){
			$this->db->query("UPDATE " . DB_PREFIX . "order SET utm_source = '" . $this->db->escape($this->request->cookie['utm_source']) . "'
							 WHERE order_id = '" . (int)$order_id. "'");
		}
		
		if (isset($this->request->cookie['utm_medium'])){
			$this->db->query("UPDATE " . DB_PREFIX . "order SET utm_medium = '" . $this->db->escape($this->request->cookie['utm_medium']) . "'
							 WHERE order_id = '" . (int)$order_id. "'");
		}
		
		if (isset($this->request->cookie['utm_campaign'])){
			$this->db->query("UPDATE " . DB_PREFIX . "order SET utm_campaign = '" . $this->db->escape($this->request->cookie['utm_campaign']) . "'
							 WHERE order_id = '" . (int)$order_id. "'");
		}
		
		if (isset($this->request->cookie['utm_content'])){
			$this->db->query("UPDATE " . DB_PREFIX . "order SET utm_content = '" . $this->db->escape($this->request->cookie['utm_content']) . "'
							 WHERE order_id = '" . (int)$order_id. "'");
		}
		
		if (isset($this->request->cookie['utm_term'])){
			$this->db->query("UPDATE " . DB_PREFIX . "order SET utm_term = '" . $this->db->escape($this->request->cookie['utm_term']) . "'
							 WHERE order_id = '" . (int)$order_id. "'");
		}
		
		if (isset($this->request->cookie['ad_name'])){
			$this->db->query("UPDATE " . DB_PREFIX . "order SET ad_name = '" . $this->db->escape($this->request->cookie['ad_name']) . "'
							 WHERE order_id = '" . (int)$order_id. "'");
		}
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			$this->send_api_order($order_id);
			
		}
	}
	
		
	public function send_api_order($order_id){
			
			$url = "https://mieportal.com/1a820d434d531";
				
			$bearer = 'b16e3c2b-4f65-4fbb-bc89-9b2743e5135b';
				
			$order_info = $this->getOrder($order_id);				
			
			$totals = array();
			$q_totals = $this->getOrderTotals($order_id);
			foreach($q_totals as $total){
				$totals[] = array('code' => $total['code'],
								  'title' => $total['title'],
								  'value' => $total['value'],
								  );
			}
			...
			foreach($order_products as $product){
				...
				$products[] = array('model' => $product['model'],
									'sku' => $product['sku'],
